...
import { Users, FileText, MessageSquare, BarChart3, TrendingUp, Clock, BookOpen } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        try {
          const formsRes = await api.get('/api/feedback-forms', { params: { page: 1, limit: 100 } });
          const items = formsRes?.data?.items || [];
          // Load subjects and compute totals per semester
          try {
            const subjRes = await api.get('/api/subjects', { params: { page: 1, limit: 1000 } });
          ...
          const counts = await Promise.all(
            items.map(async (f: any) => {
              if (!f?.slug) return 0;
              try {
                const r = await api.get(`/api/feedback-forms/${encodeURIComponent(f.slug)}/responses`, { params: { page: 1, limit: 1 } });
        ...
        try {
          const resp = await api.get('/api/staff', { params: { limit: 1 } });
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div
          className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 cursor-pointer hover:shadow-lg transition"
          onClick={() => navigate('/department-admin/feedback-generation?active=1')}
          role="button"
